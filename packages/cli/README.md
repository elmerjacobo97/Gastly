...
```bash
pnpm install
pnpm --filter @codigoconelmer/gastly-cli build
...
```bash
# Auto-read from the web app's env file (run from the repo root)
gastly-cli init --from-env apps/web/.env.local
...
```bash
pnpm --filter @codigoconelmer/gastly-cli test
```
