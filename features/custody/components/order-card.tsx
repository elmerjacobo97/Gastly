...
import {
  DepositButton,
  DisbursementButton,
} from "@/features/custody/components/record-movement-dialog";
...
export function OrderCard({
  ...
  const pctProgress =
    order.targetAmount && order.targetAmount > 0
      ? Math.min(
          100,
          Math.round((order.totalDeposited / order.targetAmount) * 100),
        )
      : null;
  ...
  return (
    <Card>
        ...
        <div className="min-w-0 flex-1">
          ...
          {order.targetAmount != null && order.targetAmount > 0 && (
            <p className="mt-1 text-sm font-semibold tabular-nums">
              Objetivo: {formatCurrency(order.targetAmount)}
            </p>
          )}
          {order.expectedOn && (
      ...
      <CardContent className="flex flex-col gap-3">
        {pctProgress != null && (
          <Progress value={pctProgress} className="[&>div]:bg-primary" />
        )}
