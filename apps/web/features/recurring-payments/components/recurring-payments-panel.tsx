...
export function RecurringPaymentsPanel({
  ...
  return (
    <main className="flex flex-1 flex-col gap-6 p-4 md:p-6">
      <section className="flex flex-col gap-3 rounded-xl border bg-card p-5 shadow-sm md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Usa montos estimados para planificar y registra el monto real cuando
            pagues.
          </p>
        </div>
      ...
      {payments.length > 0 && (
        <RecurringPaymentSummaryCards payments={payments} monthKey={monthKey} />
      )}
      ...
      <RecurringPaymentHistoryDialog
        ...
        history={
          historyPayment ? (historyByPaymentId[historyPayment.id] ?? []) : []
        }
        open={!!historyPayment}
