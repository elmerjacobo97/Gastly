    ...
    <div className="grid gap-3 sm:grid-cols-3">
      <Card size="sm">
        <CardHeader>
          ...
          <CardAction>
            <div
              className={`grid size-8 shrink-0 place-items-center rounded-lg ${allPaid ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-muted/50 text-muted-foreground"}`}
            >
        ...
        <CardContent>
          {allPaid && (
            <p className="text-xs text-muted-foreground">todo pagado</p>
          )}
          <p
            className={`text-lg font-semibold tabular-nums ${allPaid ? "text-emerald-600 dark:text-emerald-400" : "text-foreground"}`}
          >
