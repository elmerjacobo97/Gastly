...
function getLastPaymentDate(
  ...
  if (!firstPaymentOn || !totalInstallments || totalInstallments < 2)
    return null;
  try {
    const first = new Date(`${firstPaymentOn}T12:00:00`);
    return format(addMonths(first, totalInstallments - 1), "MMMM yyyy", {
      locale: es,
    });
  } catch {
    return null;
        ...
        <div className="rounded-lg bg-muted/50 px-3 py-2 text-sm">
          ...
          {interest > 0 && (
            <span className="text-muted-foreground">
              {" "}
              · Intereses: S/ {interest.toFixed(2)}
            </span>
          )}
          {lastPaymentLabel && (
            <span className="text-muted-foreground">
              {" "}
              · Último pago: {lastPaymentLabel}
            </span>
          )}
        </div>
      )}
  ...
  return (
    <>
      <Controller
        ...
        render={({ field, fieldState }) => (
          <Field data-invalid={fieldState.invalid}>
            <FieldLabel htmlFor="ei-first-payment">
            ...
            <Field data-invalid={fieldState.invalid}>
              ...
              <NumberInput
                ...
                onChange={(event) => {
                  ...
                  const nextPayment = new Date(
                    `${getNextPaymentDefault()}T12:00:00`,
                  );
                  form.setValue(
                    "firstPaymentOn",
                    format(subMonths(nextPayment, paid), "yyyy-MM-dd"),
                  );
                }}
              />
              {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
              {totalInstallments > 0 && (alreadyPaid ?? 0) > 0 && (
                <p className="text-xs text-muted-foreground">
                  ...
                  {totalInstallments - (alreadyPaid ?? 0) !== 1 ? "s" : ""} por
                  pagar.
                </p>
              )}
            </Field>
...
export function EditInstallmentDialog({
  ...
  const trackedPaidCount = purchase.payments.filter(
    (p) => !!p.transactionId,
  ).length;
  const canEditFinancials = trackedPaidCount === 0;
  const externallyPaidCount = purchase.payments.filter(
    (p) => p.paidExternally,
  ).length;
  ...
  const {
    description,
    category,
    installmentAmount: purchaseInstallmentAmount,
    totalInstallments: purchaseTotalInstallments,
    interestAmount: purchaseInterestAmount,
    firstPaymentOn: purchaseFirstPaymentOn,
    accountId,
    notes,
  } = purchase;
  ...
  const defaults = useMemo(
    () => ({
      ...
      totalAmount:
        Math.round(
          (purchaseInstallmentAmount * purchaseTotalInstallments -
    ...
    [
      description,
      categoryId,
      purchaseInstallmentAmount,
      purchaseTotalInstallments,
      purchaseInterestAmount,
      purchaseFirstPaymentOn,
      externallyPaidCount,
      accountId,
      notes,
    ],
  ...
  const form = useForm<InstallmentPurchaseValues>({
    resolver: zodResolver(
      installmentPurchaseSchema,
    ) as Resolver<InstallmentPurchaseValues>,
  ...
  const installmentAmount =
    total > 0 && totalInstallments > 0
      ...
      : 0;
  const lastPaymentLabel = getLastPaymentDate(
    firstPaymentOn,
    totalInstallments,
  );
