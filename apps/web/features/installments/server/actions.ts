...
function buildPaymentRows(
  ...
  const lastAmount =
    Math.round(
      (grandTotal - installmentAmount * (totalInstallments - 1)) * 100,
    ) / 100;
...
export async function createInstallmentPurchase(
  rawValues: InstallmentPurchaseValues,
): Promise<void> {
  const values = parseOrThrow(installmentPurchaseSchema, rawValues);
...
export async function updateInstallmentPurchase(
  ...
  const id = parseOrThrow(installmentPurchaseIdSchema, rawId);
  const values = parseOrThrow(installmentPurchaseSchema, rawValues);
  const paidCount = parseOrThrow(paidInstallmentsCountSchema, rawPaidCount);
...
export async function deleteInstallmentPurchase(rawId: string): Promise<void> {
  const id = parseOrThrow(installmentPurchaseIdSchema, rawId);
  const { supabase } = await requireUser();
...
export async function payMonthInstallments(
  ...
  rawOccurredOn: string,
): Promise<void> {
  const pending = parseOrThrow(pendingInstallmentsSchema, rawPending);
  const { occurredOn } = parseOrThrow(payInstallmentsSchema, {
    occurredOn: rawOccurredOn,
  });
  const { supabase, userId } = await requireUser();
