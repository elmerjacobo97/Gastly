...
export async function createLoan(rawValues: LoanValues): Promise<void> {
  const values = parseOrThrow(loanSchema, rawValues);
...
export async function updateLoanPerson(
  rawIds: string[],
  rawValues: EditLoanPersonValues,
): Promise<void> {
  const ids = parseOrThrow(loanIdsSchema, rawIds);
  const values = parseOrThrow(editLoanPersonSchema, rawValues);
...
export async function deleteLoanBalances(rawIds: string[]): Promise<void> {
  const ids = parseOrThrow(loanIdsSchema, rawIds);
  const { supabase } = await requireUser();
...
export async function recordLoanPayment(
  rawLoanId: string,
  rawDisbursementId: string,
  rawValues: LoanPaymentValues,
): Promise<void> {
  const loanId = parseOrThrow(loanIdSchema, rawLoanId);
  const disbursementId = parseOrThrow(
    loanDisbursementIdSchema,
    rawDisbursementId,
  );
  const values = parseOrThrow(loanPaymentSchema, rawValues);
  const { supabase } = await requireUser();
...
export async function updateLoanPayment(
  ...
  const id = parseOrThrow(loanPaymentIdSchema, rawId);
  const values = parseOrThrow(loanPaymentSchema, rawValues);
  const { supabase } = await requireUser();
...
export async function deleteLoanPayment(rawId: string): Promise<void> {
  const id = parseOrThrow(loanPaymentIdSchema, rawId);
  const { supabase } = await requireUser();
...
export async function updateLoanDisbursement(
  rawId: string,
  rawValues: LoanDisbursementValues,
): Promise<void> {
  const id = parseOrThrow(loanDisbursementIdSchema, rawId);
  const values = parseOrThrow(loanDisbursementSchema, rawValues);
  const { supabase } = await requireUser();
...
export async function deleteLoanDisbursement(rawId: string): Promise<void> {
  const id = parseOrThrow(loanDisbursementIdSchema, rawId);
  const { supabase } = await requireUser();
